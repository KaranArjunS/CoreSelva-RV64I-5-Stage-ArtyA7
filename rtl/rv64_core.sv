...
assign pc_valid    = (^pc !== 1'bx) && (pc[63:12] == 0);
...
assign addr_valid = (^ex_mem.alu_out !== 1'bx) &&
                    (ex_mem.alu_out[63:13] == 0);
assign off        = ex_mem.alu_out[2:0];
assign rdata      = addr_valid ? dmem[ex_mem.alu_out[11:3]] : 64'd0;

// Sub-word slice extraction (combinational)
always_comb begin
...
// Store merge (combinational)
always_comb begin
    w_next = rdata;
    if (ex_mem.mem_write && addr_valid) begin
        case (ex_mem.funct3)
...
always_comb begin
    mem_data_comb = 64'd0;
    if (ex_mem.mem_read && addr_valid) begin
        case (ex_mem.funct3)
            3'b000: mem_data_comb = {{56{mem_byte[7]}},  mem_byte};    // LB
            3'b100: mem_data_comb = {56'd0,               mem_byte};   // LBU
            3'b001: mem_data_comb = {{48{mem_hword[15]}}, mem_hword};  // LH
            3'b101: mem_data_comb = {48'd0,               mem_hword};  // LHU
            3'b010: mem_data_comb = {{32{mem_word[31]}},  mem_word};   // LW
            3'b110: mem_data_comb = {32'd0,               mem_word};   // LWU
            3'b011: mem_data_comb = rdata;                             // LD
            default: mem_data_comb = 64'd0;
        endcase
    end
end

// Store write - still registered (synchronous SRAM write port)
always_ff @(posedge clk) begin
    if (ex_mem.mem_write && addr_valid)
...
always_comb begin
    ...
    if (ex_mem.jump && (^ex_mem.target !== 1'bx)) begin
        flush   = 1;
        pc_next = ex_mem.target;
    end
    else if (ex_mem.branch && ex_mem.branch_taken &&
             (^ex_mem.target !== 1'bx)) begin
        flush   = 1;
        pc_next = ex_mem.target;
    end
end
